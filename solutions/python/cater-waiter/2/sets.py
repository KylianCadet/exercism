...
def clean_ingredients(dish_name: str, dish_ingredients: list[str]) -> tuple[str, list[str]]:
...
def check_drinks(drink_name: str, drink_ingredients: list[str]) -> str:
    ...
    if any(drink in ALCOHOLS for drink in drink_ingredients):
...
def tag_special_ingredients(dish: tuple[str, list[str]]) -> tuple[str, list[str]]:
...
def compile_ingredients(dishes: list[set[str]]) -> set[str]:
    ...
    """
    return functools.reduce(lambda acc, x: acc.union(x), dishes, set())
...
def singleton_ingredients(dishes: list[set[str]], intersection: set[str]) -> set[str]:
    ...
    return functools.reduce(lambda acc, x: acc.symmetric_difference(x), dishes, set()) - intersection
